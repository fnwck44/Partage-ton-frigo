...
def affiche(aliments, filter, order):
    ...
    # if filter == "sec":
    #     aliments = aliments.query.filter_by(frais='sec')
    # elif filter == "frais":
    #     aliments = aliments.query.filter_by(frais='frais')
    # elif filter == "ok":
    #     aliments = aliments.query.filter(Aliment.peremption > ajd)
    # elif filter == "perime":
    #     aliments = aliments.filter(Aliment.peremption < ajd)
    #
    # if order == "date+":
    #     aliments = aliments.query.order_by(desc(Aliment.id))
    # elif order == "date-":
    #     aliments = aliments.query.order_by(Aliment.id)
    # elif order == "dlc+":
    #     aliments = aliments.query.order_by(Aliment.peremption)
    # elif order == "dlc-":
    #     aliments = aliments.query.order_by(desc(Aliment.peremption))
    # elif order == "name":
    #     aliments = aliments.query.order_by(Aliment.titre)
    # else:
    #     aliments = aliments.query.order_by(desc(Aliment.ajout))
...
def list():
    ...
    if res != None :
        aliments = find(res, aliments)
    else :
        filter = request.form.get("filter")
        order = request.form.get("order")
        aliments = affiche(aliments, filter, order)
    return render_template("index.html", search=res, filter=filter, order=order, aliments=aliments, now=now,
                           twodays=twodays)
